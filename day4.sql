...
--1How many unique nodes are there on the Data Bank system?
...
SELECT DISTINCT
    ...
    PERCENTILE_CONT(0.5) 
    WITHIN GROUP (ORDER BY days)
    OVER (PARTITION BY region_name) AS median_days,

    PERCENTILE_CONT(0.8) 
    WITHIN GROUP (ORDER BY days)
    OVER (PARTITION BY region_name) AS p80_days,

    PERCENTILE_CONT(0.95) 
    WITHIN GROUP (ORDER BY days)
    OVER (PARTITION BY region_name) AS p95_days
...
AND prev_balance <> 0
AND (closing_balance - prev_balance) * 1.0 / NULLIF(prev_balance,0) > 0.05;
