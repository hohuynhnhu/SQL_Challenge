...
SELECT 
    COUNT(DISTINCT s.customer_id) AS customers_upgraded_annual
FROM subscriptions s
JOIN plans p
    ON s.plan_id = p.plan_id
...
SELECT
    CONCAT(
        (days_to_upgrade / 30) * 30,
        '-',
        (days_to_upgrade / 30) * 30 + 30
    ) AS day_range,
    COUNT(*) AS customer_count
FROM days_to_annual
GROUP BY (days_to_upgrade / 30)
ORDER BY (days_to_upgrade / 30);
...
SELECT
    ph.customer_id,
    ph.plan_id,
    DATEADD(month, m.m-1, ph.start_date) AS payment_date
FROM plan_history ph
JOIN months m
ON DATEADD(month, m.m-1, ph.start_date)
    BETWEEN ph.start_date
    AND ISNULL(ph.next_plan_date,'2020-12-31')
)
...
SELECT
    month_start,
    new_customers,
    LAG(new_customers) OVER (ORDER BY month_start) AS previous_month,
    ROUND(
        100.0 * (new_customers - LAG(new_customers) OVER (ORDER BY month_start))
        / LAG(new_customers) OVER (ORDER BY month_start),
        2
...
SELECT 
    COUNT(DISTINCT p.customer_id) AS converted_users,
    ROUND(
        100.0 * COUNT(DISTINCT p.customer_id) /
...
SELECT 
    COUNT(DISTINCT customer_id) AS upgrade_count
FROM plan_history
WHERE previous_plan = 1
AND plan_id IN (2,3);
...
SELECT
    ...
    MIN(CASE WHEN plan_id IN (1,2,3) THEN start_date END) AS paid_date
FROM subscriptions
GROUP BY customer_id
)

SELECT 
AVG(DATEDIFF(day, trial_date, paid_date)) AS avg_days_to_paid
FROM customer_dates
